...
export default function Photo({ label, id, date, thumbnail, title, length }) {
    const navigate = useNavigate();

    if (!date) return <Skeleton className="photo" />;

    if (label === "none") return <img id={id} alt="" src={`/photography_data/${date}/${thumbnail}`} className="photo" />;

    return <button id={id} className="notbutton photo"
